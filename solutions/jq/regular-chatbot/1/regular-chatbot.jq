...
def check_phone_number:
  if (. | test("\\(\\+\\d{2}\\)\\s\\d{3}-\\d{3}-\\d{3}"))
  then "Thanks! Your phone number is OK."
  else (capture("(?<number>[0-9\\-]+)") | "Oops, it seems like I can't reach out to \(.number).")
  end
;
...
def nice_to_meet_you:
  capture("My name is (?<name>[\\w-]+\\b)"; "gim") | "Nice to meet you, \(.name)"
;
