(module
  ;;
  ;; Determine if a year is a leap year
  ;;
  ;; @param {i32} year - The year to check
  ;;
  ;; @returns {i32} 1 if leap year, 0 otherwise
  ;;
  (func (export "isLeap") (param $year i32) (result i32)
    ;; Check if it is divisible by 4 returns false if not
    (if (i32.ne (i32.rem_u (local.get $year) (i32.const 4)) (i32.const 0))
      (then (return (i32.const 0))))
    
    ;; Check if it is divisible by 100 returns true if not
    (if (i32.ne (i32.rem_u (local.get $year) (i32.const 100)) (i32.const 0))
      (then (return (i32.const 1))))
    
    ;; Check if it is divisible by 400 returns false if not
    (i32.eqz 
      (i32.rem_u (local.get $year) (i32.const 400)))
  )  
)
